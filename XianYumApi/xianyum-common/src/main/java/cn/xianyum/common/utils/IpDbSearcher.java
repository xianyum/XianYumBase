...
import cn.xianyum.common.config.IpDbConfig;
...
public class IpDbSearcher {
     ...
     * db config
     */
    private IpDbConfig dbConfig = null;
     ...
     */
    public IpDbSearcher( IpDbConfig dbConfig, String dbFile ) throws FileNotFoundException
    {
     ...
     * @param   dbBinStr
     */
    public IpDbSearcher(IpDbConfig dbConfig, byte[] dbBinStr)
    {
        this.dbConfig = dbConfig;
        this.dbBinStr = dbBinStr;

        firstIndexPtr = IPUtils.getIntLong(dbBinStr, 0);
        lastIndexPtr  = IPUtils.getIntLong(dbBinStr, 4);
        totalIndexBlocks = (int)((lastIndexPtr - firstIndexPtr)/IpIndexBlock.getIndexBlockLength()) + 1;
    }
     ...
     * @throws  IOException
     */
    public IpDataBlock memorySearch(long ip) throws IOException
    {
        int blen = IpIndexBlock.getIndexBlockLength();
        if ( dbBinStr == null ) {
        ...
        while ( l <= h ) {
            ...
            sip = IPUtils.getIntLong(dbBinStr, p);
            if ( ip < sip ) {
                h = m - 1;
            } else {
                eip = IPUtils.getIntLong(dbBinStr, p + 4);
                if ( ip > eip ) {
                    l = m + 1;
                } else {
                    dataptr = IPUtils.getIntLong(dbBinStr, p + 8);
                    break;
                }
            }
        }
        ...
        int city_id = (int)IPUtils.getIntLong(dbBinStr, dataPtr);
        String region = new String(dbBinStr, dataPtr + 4, dataLen - 4, "UTF-8");

        return new IpDataBlock(city_id, region, dataPtr);
    }
     ...
     * @throws  IOException
     */
    public IpDataBlock memorySearch( String ip ) throws IOException
    {
        return memorySearch(IPUtils.ip2long(ip));
    }
     ...
     * @throws  IOException
     */
    public IpDataBlock getByIndexPtr( long ptr ) throws IOException
    {
        raf.seek(ptr);
        ...
        long extra = IPUtils.getIntLong(buffer, 8);
        ...
        int city_id = (int)IPUtils.getIntLong(data, 0);
     ...
     * @throws  IOException
     */
    public IpDataBlock btreeSearch( long ip ) throws IOException
    {
        ...
        if ( HeaderSip == null )  {
            ...
            for ( int i = 0; i < b.length; i += 8 ) {
                startIp = IPUtils.getIntLong(b, i);
                dataPtr = IPUtils.getIntLong(b, i + 4);
                if ( dataPtr == 0 ) break;
        ...
        int blockLen = eptr - sptr, blen = IpIndexBlock.getIndexBlockLength();
        ...
        while ( l <= h ) {
            int m = (l + h) >> 1;
            int p = m * blen;
            sip = IPUtils.getIntLong(iBuffer, p);
            if ( ip < sip ) {
                h = m - 1;
            } else {
                eip = IPUtils.getIntLong(iBuffer, p + 4);
                if ( ip > eip ) {
                    l = m + 1;
                } else {
                    dataptr = IPUtils.getIntLong(iBuffer, p + 8);
                    break;
                }
            }
        }
     ...
     * @throws  IOException
     */
    public IpDataBlock btreeSearch( String ip ) throws IOException
    {
        return btreeSearch(IPUtils.ip2long(ip));
    }
     ...
     * @throws  IOException
     */
    public IpDataBlock binarySearch( long ip ) throws IOException
    {
        int blen = IpIndexBlock.getIndexBlockLength();
        if ( totalIndexBlocks == 0 ) {
            ...
            //initialize the global vars
            firstIndexPtr = IPUtils.getIntLong(superBytes, 0);
            lastIndexPtr = IPUtils.getIntLong(superBytes, 4);
            totalIndexBlocks = (int)((lastIndexPtr - firstIndexPtr)/blen) + 1;
        }
        ...
        while ( l <= h ) {
            ...
            raf.readFully(buffer, 0, buffer.length);
            sip = IPUtils.getIntLong(buffer, 0);
            if ( ip < sip ) {
                h = m - 1;
            } else {
                eip = IPUtils.getIntLong(buffer, 4);
                if ( ip > eip ) {
                    l = m + 1;
                } else {
                    dataptr = IPUtils.getIntLong(buffer, 8);
                    break;
                }
            }
        }
     ...
     * @throws  IOException
     */
    public IpDataBlock binarySearch( String ip ) throws IOException
    {
        return binarySearch(IPUtils.ip2long(ip));
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        IpDbSearcher searcher = new IpDbSearcher(config, "E:\\MyProjects\\ip2region\\data\\ip2region.db");
        IpDataBlock dataBlock = searcher.btreeSearch("47.96.19.72");
        String[] data = dataBlock.getRegion().split("\\|");
        IpInfoEntity ipInfoEntity = new IpInfoEntity();
        ipInfoEntity.setCountry(data[0]);
        ipInfoEntity.setProv(data[2]);
        ipInfoEntity.setCity(data[3]);
        ipInfoEntity.setIsp(data[4]);
        System.out.println(ipInfoEntity);
    }
}
